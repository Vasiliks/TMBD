...
for path in glob.glob(os.path.join(ROOT_DIR, 'modules', '*.whl')): sys.path.insert(0, path)

from . import _, isDreamOS
from Plugins.Plugin import PluginDescriptor
from twisted.web.client import downloadPage
from enigma import gPixmapPtr,ePicLoad, eServiceReference, eTimer, eServiceCenter, getDesktop, ePoint, eSize, iPlayableService, eListbox
...
import re, urllib, array, struct, fcntl, shutil, urllib2
import tmdb
from tmdb import cutName
from tmdbYTT import tmbdYTTrailer
from socket import socket, AF_INET, SOCK_DGRAM
from event import Event, ShortEventDescriptor, ExtendedEventDescriptor
from time import strftime, localtime, mktime, gmtime
...
def getYear(event):
	...
	yr = [_y for _y in re.findall(r'\d{4}', descr) if '1930' <= _y <= '%s' % gmtime().tm_year]
...
class TMBD(Screen):
	skin_hd1 = """
		...
		<widget name="starsbg" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_empty.png" position="790,10" size="210,21"  alphatest="blend" transparent="1" zPosition="2" />
		<widget name="stars" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_filled.png" position="790,10"  size="210,21" transparent="1" zPosition="3" />
		...
		<widget name="menu" position="325,100" scrollbarMode="showOnDemand" size="750,130" zPosition="3"  selectionPixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/button1080x25.png" />
		...
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/red25.png" position=" 20,532" size="250,38" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/green25.png" position="290,532" size="250,38" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/yellow25.png" position="560,532" size="250,38" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/blue25.png" position="830,532" size="250,38" zPosition="1" />
	...
	skin_hd = """
		...
		<widget name="menu" position="170,100" scrollbarMode="showOnDemand" size="920,130" zPosition="3" selectionPixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/button1080x25.png" />
		<widget name="detailslabel" position="325,59" size="570,35" font="Regular;19" transparent="1" />
	...
	skin_fullhd = """
		...
		<widget name="starsbg" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_empty.png" position="1185,15" size="210,21" alphatest="blend"  transparent="1" zPosition="2" />
		<widget name="stars" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_filled.png" position="1185,15"  size="210,21" transparent="1" zPosition="3" />
		...
		<widget name="menu" position="487,150" scrollbarMode="showOnDemand" size="1125,195" zPosition="3"  selectionPixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/button1080x25.png" />
		...
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/red25.png" position="30,798" size="375,57" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/green25.png" position="435,798" size="375,57" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/yellow25.png" position="840,798" size="375,57" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/blue25.png" position="1245,798" size="375,57" zPosition="1" />
	...
	skin_sd = """
		...
		<widget name="starsbg" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_empty.png" position="340,40" zPosition="0" size="210,21" transparent="1" alphatest="on" />
		<widget name="stars" position="340,40" size="210,21" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_filled.png" transparent="1" />
	</screen>"""

	def __init__(self, session, eventName, eventYear=None, callbackNeeded=False, movielist=False):
		...
		if isDreamOS:
			self.refreshTimer_conn = self.refreshTimer.timeout.connect(self.TMBDPoster)
		else:
		...
		if self.tmdb3 is None:
			self["statusbar"].setText(_("TMBD init error!"))
			return
	...
	def TestConnection(self):
		...
		try:
			testOK = bool([(s.connect(('1.1.1.1', 0)), s.getsockname()[0], s.close()) for s in [socket(AF_INET, SOCK_DGRAM)]][0][1])
		except:
			testOK = False
			self.resetLabels()
	...
	def aboutAutor(self):
		self.session.open(MessageBox, _("TMBD Plugin Details\nDevelopers: Nikolasi,vlamo,Dima73,Dorik1972\n(c)2012-2020"), MessageBox.TYPE_INFO)
			...
			try:
				...
				if name != '':
					try:
						...
						pass
				try:
					released = movie.releasedate.year  # For Movies
				except:
					try:
						released = movie.first_air_date.year  #For Series
					except:
						released = ''
				if released:
					try:
						...
						pass
				try:
					runtime = movie.runtime    # For Movies
				except:
					try:
						runtime = movie.episode_run_times   #For Series
					except:
						runtime = ''
				if runtime:
					try:
		...
		if self.curResult:
			...
			if current:
				...
				try:
					rel = movie.releases
					if rel:
						certification = str(tmdb.decodeCertification(rel, self.tmdb3.get_locale().country))
					else:
						certification = ''
				except:
					certification = ''
	...
	def writeMeta(self):
		if self.curResult:
			...
			if current:
				...
				self.timer = eTimer()
				if isDreamOS:
					self.timer_conn = self.timer.timeout.connect(self.savePosterInfo)
				else:
	...
	def searchYttrailer(self):
		if self.curResult:
			...
			if current:
				try:
					yr = current[1].first_air_date.year
				except:
					try:
						yr = current[1].releasedate.year
					except:
						yr = self.eventYear if not None else 1990

				self.session.open(tmbdYTTrailer.TmbdYTTrailerList, current[0].strip(), yr)
	...
	def getSeries(self, title):
		...
		return results or self.tmdb3.searchSeries(title.split('.')[0])
	...
	def getTMBD(self):
		...
		if self.eventName:
			if not self.TVseries and re.match(r'((Т|т|T)/(С|с|C|c))', self.eventName):
				self.TVseries = True
			title = cutName(self.eventName)
			try:
				results = self.getSeries(title) if self.TVseries else self.tmdb3.searchMovie(title, adult=True, year=self.eventYear)
				if not (results or self.TVseries):
                                       	results = self.getSeries(title)
				...
				self.resultlist = sorted([(x.title.encode('utf-8', 'ignore'), x) for x in results], key=lambda x: [int(c) if c.isdigit() else c for c in re.split(r'(\d+)', x[0])])
				total = len(results)
			except:
				self.resultlist = []
			else:
	...
	def TMBDPoster(self):
		...
		if fileExists("/tmp/preview.jpg"):
			...
			jpg_file = resolveFilename(SCOPE_PLUGINS, "Extensions/TMBD/img/picon_default.png")
		...
		if not isDreamOS:
			...
			self.picload.startDecode(jpg_file, False)
	...
	if screenWidth >= 1920:
		skin = """
			<screen position="center,center" size="1110,727" title="TMBDSettings" backgroundColor="#31000000" >
				...
				<ePixmap pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/red25.png" position="15,675" size="352,66" zPosition="1" alphatest="on" />
				<ePixmap pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/green25.png" position="727,675" size="352,66" zPosition="1" alphatest="on" />
			</screen>"""
	else:
		skin = """
			<screen position="center,center" size="740,485" title="TMBDSettings" backgroundColor="#31000000" >
				...
				<ePixmap pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/red25.png" position="10,450" size="235,44" zPosition="1" alphatest="on" />
				<ePixmap pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/green25.png" position="485,450" size="235,44" zPosition="1" alphatest="on" />
			</screen>"""

	def __init__(self, session, args = None):
		Screen.__init__(self, session)
		self.setTitle(_("Settings TMBD Details Plugin...") + " " + __version__)
...
class KinoRu(Screen):
	skin_hd1 = """
		...
		<widget alphatest="blend" name="starsbg" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_empty.png" position="770,10" size="210,21" zPosition="2" />
		<widget name="stars" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_filled.png" position="770,10"  size="210,21" transparent="1" zPosition="3" />
		...
		<widget alphatest="blend" name="poster" position="30,60" size="285,398" />
		<widget name="menu" position="20,270" foregroundColor="#00f0b400" scrollbarMode="showOnDemand" size="1100,200" zPosition="1"  selectionPixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/button1080x25.png" />
		...
		<widget font="Regular;18" name="statusbar" position="10,490" size="1080,20" transparent="1" />
		<widget name="kino" alphatest="blend" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/kino_ru.png" transparent="1" position="30,90" size="350,300" zPosition="1" />
		<eLabel backgroundColor="#00bbbbbb" position="0,518" size="1100,2" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/red25.png" position=" 20,532" size="250,38" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/green25.png" position="290,532" size="250,38" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/yellow25.png" position="560,532" size="250,38" zPosition="1" />
	...
	skin_fullhd = """
		...
		<widget alphatest="blend" name="starsbg" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_empty.png" position="1155,15" size="210,21" zPosition="2" />
		<widget name="stars" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/starsbar_filled.png" position="1155,15"  size="210,21" transparent="1" zPosition="3" />
		<widget font="Regular;30" halign="left" name="ratinglabel" foregroundColor="#00f0b400" position="1155,51" size="210,23" transparent="1" />
		<widget font="Regular;30" name="voteslabel" halign="left" position="1155,85" size="495,34" foregroundColor="#00f0b400" transparent="1" />
		<widget alphatest="blend" name="poster" position="45,90" size="427,597" />
		<widget name="menu" position="30,405" foregroundColor="#00f0b400" scrollbarMode="showOnDemand" size="1650,300" zPosition="1"  selectionPixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/button1080x25.png" />
		...
		<widget font="Regular;27" name="statusbar" position="15,735" size="1620,30" transparent="1" />
		<widget name="kino" alphatest="blend" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/kino_ru.png" transparent="1" position="45,135" size="525,450" zPosition="1" />
		<eLabel backgroundColor="#00bbbbbb" position="0,777" size="1650,3" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/red25.png" position="30,798" size="375,57" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/green25.png" position="435,798" size="375,57" zPosition="1" />
		<ePixmap alphatest="on" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/yellow25.png" position="840,798" size="375,57" zPosition="1" />
	...
	skin_sd = """
		...
		<widget name="kino" alphatest="blend" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/TMBD/img/kino_smail.png" transparent="1" position="10,110" size="110,170" zPosition="1" />
	...
	def __init__(self, session, eventName, callbackNeeded=False, movielist=False):
		...
		self.refreshTimer = eTimer()
		if isDreamOS:
			self.refreshTimer_conn = self.refreshTimer.timeout.connect(self.KinoRuPoster)
		else:
			...
			if sel:
				...
				if id is not None and id != 'n\a':
					if id != 'n\x07':
						...
						Ratingtext = ''
						if rating != '':
							Ratingtext = _("User Rating") + ": %s  / 10" % rating
							self.ratingstars = int(10*round(float(rating.replace(',','.')),1))
							self["starsbg"].show()
							self["stars"].setValue(self.ratingstars)
							self["stars"].show()
	...
	def searchYttrailer(self):
		if self.curResult:
			current = self["menu"].l.getCurrentSelection()
			if current:
				self.session.open(tmbdYTTrailer.TmbdYTTrailerList, current[0])
	...
	def paintPosterPixmapCB(self, picInfo=None):
		ptr = self.picload.getData()
		if ptr and self.curResult and self.curPoster:
			self["poster"].instance.setPixmap(ptr)
			self["poster"].show()
		else:
			self["poster"].hide()
		...
		if not self.working:
			if movie and self.mayShow and config.plugins.tmbd.enabled.value:
				...
				if fileExists(png):
					self.working = True
					sc = AVSwitch().getFramebufferScale()
					size = [int(x) for x in config.plugins.tmbd.size.value.split("x")]
					self.picload.setPara((size[0], size[1], sc[0], sc[1], False, 1, "#00000000"))
					if not isDreamOS:
...
class EventChoiseList:
	def __init__(self, session):
		...
		if event_now:
			eventname_now = event_now.getEventName()
		event_next = info and info.getEvent(1)	# 1 = next

		if event_next:
			eventname_next = event_next.getEventName()
...
def eventinfo(session, eventName='', event=None, **kwargs):
	if eventName:
		if config.plugins.tmbd.profile.value == "0":
			session.open(TMBD, eventName, getYear(event))
		else:
...
def yteventinfo(session, eventName='', **kwargs):
	...
	if eventName:
		...
		event_now = info and info.getEvent(0)
		if event_now:
			eventName = cutName(event_now.getEventName())
			if config.plugins.tmbd.yt_start.value == "0":
				session.open(tmbdYTTrailer.TmbdYTTrailerList, eventName)
			else:
				ytTrailer = tmbdYTTrailer.tmbdYTTrailer(session)
